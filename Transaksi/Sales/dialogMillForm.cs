...
namespace Sales
{
    public partial class dialogMillForm : Syncfusion.Windows.Forms.MetroForm
    {
        ...
        public dialogMillForm(string username, string typeForm, string id = "")
        {
        ...
        private void btnSearch_Click(object sender, EventArgs e)
        {
            ...
            if (cmbFilterColumn.SelectedItem.ToString() == "Nama Pabrik")
            {
                name = txtFilterValue.Text;
            }
            else if (cmbFilterColumn.SelectedItem.ToString() == "Kode Pabrik")
            {
        ...
        public void Grid_Load(string code = "", string name = "", string address = "", string note = "")
        {
            ...
            dt.Columns.Add("Kode Pabrik", typeof(String));
            dt.Columns.Add("Nama Pabrik", typeof(String));
            ...
            try
            {
                String sqlString = "SELECT id, code, name, address, phone, handphone, fax, email, note, status, username, updated, created FROM masterMill WHERE " +
                    "status = 1 " +
                ...
                using (SqlDataAdapter da = new SqlDataAdapter(sqlString, connString))
                {
                    ...
                    using (DataSet ds = new DataSet())
                    {
                        ...
                        for (int i = 0; i < tDT.Rows.Count; i++)
                        {
                            ...
                            dr["ID"] = tDR["id"];
                            dr["Kode Pabrik"] = tDR["code"];
                            dr["Nama Pabrik"] = tDR["name"];
        ...
        {
            cmbFilterColumn.Items.Add("Nama Pabrik");
            cmbFilterColumn.Items.Add("Kode Pabrik");
            cmbFilterColumn.Items.Add("Alamat");
            cmbFilterColumn.Items.Add("Keterangan");
            cmbFilterColumn.SelectedItem = "Nama Pabrik";
        }
        #endregion
        ...
        private void gridView_CellDoubleClick(object sender, DataGridViewCellEventArgs e)
        {
            if (gridView.SelectedRows.Count > 0)
            {
                if (typeForm == "NewForm")
                {
                    ...
                    ((NewForm)this.Owner).btnMillClick(code);
                    this.Close();
                }
                else if (typeForm == "EditForm")
                {
                    ...
                    ((EditForm)this.Owner).btnMillClick(code);
                    this.Close();
                }
            }
            else
