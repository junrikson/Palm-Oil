...
namespace Receipt
{
    public partial class MainForm : Syncfusion.Windows.Forms.MetroForm
    {
        ...
        private void btnDelete_Click(object sender, EventArgs e)
        {
            ...
            if (gridView.SelectedCells.Count != 0)
            {
                try
                {
                    ...
                    DataGridViewRow row = cell.OwningRow;
                    id = row.Cells[0].Value.ToString();
                    name = row.Cells["Kode Tanda Terima"].Value.ToString();
                    ...
                    if (dialogResult == DialogResult.Yes)
                    {
                        try
                        {
                            using (SqlConnection con = new SqlConnection(connString))
                            {
                                using (SqlCommand cmd = new SqlCommand("DELETE FROM receipt WHERE id = @id", con))
                                {
        ...
        public void Grid_Load(string code = "", string relationCode = "", string relationName = "", string note = "")
        {
            ...
            try
            {
                String sqlString = "SELECT a.id, a.code, a.date, b.code as relationCode, b.name as relationName, a.licensePlate, a.driver, a.note, a.username, a.updated, a.created " +
