    ...
    public partial class NewForm : Syncfusion.Windows.Forms.MetroForm
    {
        ...
        private Boolean isValid()
        {
            ...
            if (isValidated == true)
            {
                ...
                {
                    isValidated = false;
                    MessageBox.Show("Nomor Pembayaran ini sudah pernah digunakan!", "Warning!", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                    txtCode.Select();
                }
            }
            return isValidated;
        ...
        private Boolean checkCode(string code)
        {
            ...
            try
            {
                using (SqlConnection con = new SqlConnection(connString))
                {
                    using (SqlCommand cmd = new SqlCommand("SELECT TOP 1 code FROM payment WHERE code = @code and ID <> @ID ", con))
                    {
        ...
        private void createDataHeader()
        {
            try
            {
                ...
                using (SqlConnection con = new SqlConnection(connString))
                {
                    using (SqlCommand cmd = new SqlCommand(query, con))
                    {
                        cmd.Parameters.Add("@code", SqlDbType.VarChar).Value = "PY/" + dtpDate.Value.ToString("yyMMdd");
        ...
        private void Load_Data()
        {
            try
            {
                using (SqlConnection con = new SqlConnection(connString))
                {
                    using (SqlCommand cmd = new SqlCommand("SELECT TOP 1 code FROM payment WHERE id = @id", con))
                    {
            ...
            if (onCloseDelete)
            {
                try
                {
                    using (SqlConnection con = new SqlConnection(connString))
                    {
                        ...
                        cmd = new SqlCommand("DELETE FROM paymentDetails WHERE paymentID = @paymentID", con, tran);
                        cmd.Parameters.Add("@paymentID", SqlDbType.VarChar).Value = this.id;
                        cmd.ExecuteNonQuery();

                        cmd = new SqlCommand("DELETE FROM payment WHERE ID = @ID", con, tran);
                        cmd.Parameters.Add("@ID", SqlDbType.VarChar).Value = this.id;
                        cmd.ExecuteNonQuery();
        ...
        private void gridView_CellEndEdit(object sender, DataGridViewCellEventArgs e)
        {
            ...
            if (!string.IsNullOrEmpty(code))
                    ...
                    else
                    {
                        try
                        {
                            string query = "INSERT INTO paymentDetails (paymentID, purchaseCode, ammount) " +
                                "SELECT TOP 1 @paymentID, code, (total-paid) FROM View_PurchasePayment WHERE code = @code";
                ...
                {
                    MessageBox.Show("Invoice tidak ditemukan atau sudah dibayar!. Pastikan nilai pembayaran tidak melebihi sisa Invoice.");
                }
        ...
        private void gridView_CellDoubleClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            Form dialogForm = new dialogPurchaseForm(username, "NewForm", this.id);
