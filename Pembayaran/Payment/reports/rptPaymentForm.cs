...
namespace Payment
{
    public partial class rptPaymentForm : Syncfusion.Windows.Forms.MetroForm
    {
        string connString = ConfigurationManager.ConnectionStrings["stowageConnection"].ToString();
        string id = "";
        string reportName = "rptPayment.rpt";

        public rptPaymentForm(string id)
        {
            this.id = id;
            InitializeComponent();
        }

        private void rptPaymentForm_Load(object sender, EventArgs e)
        {
        ...
        private Decimal getTotal(string id)
        {
            ...
            try
            {
                ...
                {
                    using (SqlCommand cmd = new SqlCommand("SELECT TOP 1 total FROM payment WHERE ID = @ID ", con))
                    {
