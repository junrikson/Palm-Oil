...
namespace Repayment
{
    public partial class rptRepaymentForm : Syncfusion.Windows.Forms.MetroForm
    {
        string connString = ConfigurationManager.ConnectionStrings["stowageConnection"].ToString();
        string id = "";
        string reportName = "rptRepayment.rpt";

        public rptRepaymentForm(string id)
        {
            this.id = id;
            InitializeComponent();
        }

        private void rptRepaymentForm_Load(object sender, EventArgs e)
        {
        ...
        private Decimal getTotal(string id)
        {
            ...
            try
            {
                ...
                {
                    using (SqlCommand cmd = new SqlCommand("SELECT TOP 1 total FROM repayment WHERE ID = @ID ", con))
                    {
