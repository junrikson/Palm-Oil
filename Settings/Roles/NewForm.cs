...
namespace Roles
{
    public partial class NewForm : Syncfusion.Windows.Forms.MetroForm
    {
        ...
        public bool OnCloseDelete { get => onCloseDelete; set => onCloseDelete = value; }
        ...
        private void createDataHeader()
        {
            try
            {
                string query = "INSERT INTO masterRoles" +
                        "(code, name, status, updated, created, username) output INSERTED.ID VALUES " +
                        ...
                        "'-', 1, getdate(), getdate(), @username)";

                using (SqlConnection con = new SqlConnection(ConnString))
                {
                    using (SqlCommand cmd = new SqlCommand(query, con))
                    {
                        cmd.Parameters.Add("@code", SqlDbType.VarChar).Value = "ROL/" + DateTime.Now.ToString("yyMMdd");
                        cmd.Parameters.Add("@today", SqlDbType.VarChar).Value = DateTime.Now.ToString("yyMMdd");
            ...
            if (OnCloseDelete)
            {
                try
                {
                    using (SqlConnection con = new SqlConnection(ConnString))
                    {
                        ...
                        cmd = new SqlCommand("DELETE FROM masterRolesDetail WHERE rolesID = @rolesID", con, tran);
                        cmd.Parameters.Add("@rolesID", SqlDbType.VarChar).Value = this.Id;
                        cmd.ExecuteNonQuery();

                        cmd = new SqlCommand("DELETE FROM masterRoles WHERE ID = @ID", con, tran);
                        cmd.Parameters.Add("@ID", SqlDbType.VarChar).Value = this.Id;
                        cmd.ExecuteNonQuery();
